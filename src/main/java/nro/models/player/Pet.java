...
//            Logger.logException(Pet.class, e);
    ...
    public void askPea() {
        if (this.typePet >= 1 && master.charms.tdDeTuMabu > System.currentTimeMillis()) {
            ...
            Service.getInstance().chatJustForMe(master, this, "Ê con lợn. Ném bố viên đậu nào?");
    ...
    public void transform() {
        if (this.typePet == 1) {
            ...
            Service.getInstance().chat(this, "Cự bặc cự bặc....");
        } else if (this.typePet == 2) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "Đồ ngu, đồ ăn hại, cút mẹ mày đi....");
        }
        if (this.typePet == 3) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "Bố mày là Đấng Zeno....");
        }
        if (this.typePet == 4) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "Từ lớp 1 đến lớp 5 tao còn chưa sợ....");
        }
        if (this.typePet == 5) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "Nhìn cái đéo gì? Bố chọc mù mắt mày giờ....");
        }
    }
    ...
    public void angry(Player plAtt) {
        ...
        if (plAtt != null) {
            ...
            Service.getInstance().chatJustForMe(master, this, "Còn mày thích thì nhảy vào ăn đi? " + playerAttack.name
                    .replace("$", ""));
        }
    }
}
