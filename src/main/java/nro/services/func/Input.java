                ...
                case QUY_DOI_COIN_1:
                    ...
                    if (goldTrade1 % 1000 == 0) {
                        if (goldTrade1 <= 0 || goldTrade1 >= 1000001) {
                            ...
                            Item thoiVang = ItemService.gI().createNewItem((short) 457, (kmnap1 * (goldTrade1 / 10)));
                            ...
                            Service.getInstance().sendThongBao(player, "|7|Số Coin của bạn là " + player.getSession().vnd + " không đủ để quy "
                                    + " đổi " + (goldTrade1 / 10) + " Thỏi vàng" + " " + "bạn cần thêm " + (player.getSession().vnd - goldTrade1));
                        }
                    } else {
                        Service.getInstance().sendThongBao(player, "|7|Số Coin nhập phải là bội số của 1000");
                    }
                ...
                case QUY_DOI_XU_VANG:
                    ...
                    if (goldTrade2 % 1000 == 0) {
                        if (goldTrade2 <= 0 || goldTrade2 >= 1000001) {
                            ...
                            Item Xuvang = ItemService.gI().createNewItem((short) 1535, (kmnap2 * (goldTrade2 / 20)));
                            InventoryService.gI().addItemBag(player, Xuvang, 9999);
                            InventoryService.gI().sendItemBags(player);
                            Service.getInstance().sendThongBao(player, "Bạn nhận được " + Util.format((kmnap2 * (goldTrade2 / 20)))
                                    + " " + Xuvang.template.name);
                        } else {
                            Service.getInstance().sendThongBao(player, "|7|Số Coin của bạn là " + player.getSession().vnd + " không đủ để quy "
                                    + " đổi " + (goldTrade2 / 20) + " Xu vàng" + " " + "bạn cần thêm " + (player.getSession().vnd - goldTrade2));
                        }
                    } else {
                        Service.getInstance().sendThongBao(player, "|7|Số Coin nhập phải là bội số của 1000");
                    }
